...
const APP_NAME = manifest.productName ?? manifest.name;
const ELECTRON_DIST = join(ROOT, "node_modules", "electron", "dist");
const OUT = join(ROOT, "dist", APP_NAME);
const EXE = join(OUT, `${APP_NAME}.exe`);
...
step(`重命名 electron.exe → ${APP_NAME}.exe`);
renameSync(join(OUT, "electron.exe"), EXE);
...
async function patchExecutable() {
  ...
  const executable = NtExecutable.from(readFileSync(EXE), { ignoreCert: true });
  ...
  versionInfo.setStringValues({ lang: 1033, codepage: 1200 }, strings);
